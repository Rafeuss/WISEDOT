...
## 2.1 Métricas de fluxo (Throughput, WIP, Lead Time, Cycle Time)

A faixa "Pronto (total)" é a curva de entrada acumulada no sistema (todo item que chegou ao fluxo, independente da etapa em que está hoje) — por isso ela só cresce e é sempre ≥ todas as demais faixas. "Implantado" é a curva de itens efetivamente concluídos (entregues). As métricas de fluxo abaixo partem dessa leitura.

**Throughput (vazão):** itens entregues por dia = variação da faixa "Implantado". Como essa faixa permanece em 0 do dia 1 ao dia 8, **o throughput observado na janela inteira é 0 itens/dia** — nenhum item completou o fluxo.

**Taxa de chegada (arrival rate):** variação diária da faixa "Pronto": +3, +2, +4, +2, +1, +2, +1, +1 (dias 1 a 8) — média de **≈2 itens/dia** entrando no sistema. Com throughput=0, o sistema acumula fila indefinidamente: a fila cresce ~2 itens/dia e não escoa nada.

**WIP (Work In Progress) no dia 8:** WIP total = Pronto − Implantado = 16 − 0 = **16 itens em andamento**, ou seja, 100% de tudo que já entrou no sistema ainda está em algum ponto do fluxo. Decompondo por etapa (distância horizontal entre faixas adjacentes no CFD, todas medidas no dia 8):

| Etapa (WIP parado ali) | Cálculo | Itens |
|---|---|---|
| Backlog puro (ainda não entrou em análise) | Pronto (16) − Em análise (11) | 5 |
| Em análise | Em análise (11) − Em desenvolvimento (8) | 3 |
| Em desenvolvimento | Em desenvolvimento (8) − Em testes (3) | 5 |
| Em teste | Em testes (3) − Implantado (0) | 3 |
| **Total (= WIP)** | | **16** |

**Cycle Time (Lei de Little: CT = WIP ÷ Throughput):** com throughput = 0, a divisão é indefinida — em 8 dias nenhum item chegou a "Implantado", então não há um cycle time médio empírico para calcular. O que dá para afirmar são limites inferiores, a partir de quanto tempo cada faixa ficou parada sem mover itens para a próxima:
- **Em análise:** os 11 itens dessa faixa não se movem do dia 5 ao dia 8 → cycle time mínimo já observado nessa etapa é **≥ 3 dias** (e crescendo a cada dia sem entrega).
- **Em desenvolvimento:** os 8 itens não se movem do dia 6 ao dia 8 → cycle time mínimo **≥ 2 dias**.
- **Em teste:** os 3 itens que chegaram (1 no dia 6, mais 1 no dia 7, mais 1 no dia 8) permanecem todos ali, sem nenhuma saída para "Implantado" → o mais antigo já acumula **≥ 2 dias** parado nessa etapa.

## 2.2 Análise dedicada: "Pronto" vs. "Entregue"

A distância entre as duas curvas mais externas do CFD — "Pronto" (0→16) e "Entregue/Implantado" (0→0) — é o próprio WIP total calculado acima. Ela cresce a cada dia (0, 3, 5, 9, 11, 12, 14, 15, 16) porque entram ~2 itens/dia (seção 2.1) e não sai nenhum pelo outro lado. Isso confirma, com números, a conclusão qualitativa da seção 3.4: o time inicia trabalho num ritmo que a etapa de entrega não acompanha — enquanto o throughput seguir zero, a fila só aumenta.
