...
async function cleanTestUsersByEmailPrefix(prefix) {
  const client = await pool.connect();
  try {
    await client.query('BEGIN');
    const { rows } = await client.query(
      `SELECT usr_st_email FROM tb_user WHERE usr_st_email LIKE $1`,
      [`${prefix}%`],
    );
    for (const row of rows) {
      await deleteUserCascade(client, row.usr_st_email);
    }
    await client.query('COMMIT');
    return rows.length;
  } catch (err) {
...
module.exports = { TEST_USER, seedTestUser, cleanTestUser, cleanTestUsersByEmailPrefix };
